...
def show_inventory_page(show_navigation_bar, navigate_to):
    ...
    if st.session_state.inventory_optimized:
        ...
        st.plotly_chart(fig, width='stretch', key="inventory_chart")
        
        col_o1, col_o2 = st.columns(2)
        
        with col_o1:
            fig = create_orders_chart(subset_test)
            st.plotly_chart(fig, width='stretch', key="orders_chart")
        
        with col_o2:
            fig = create_shipments_chart(subset_test)
            st.plotly_chart(fig, width='stretch', key="shipments_chart")
